...
# Onboard train & test .txt files into memory
# ignore the 'Inertial Signals' folders
...
if(!exists("mergedData") || debug){
        ...
        # remove the y activity test and training sets
        rm(data_y_test)
        rm(data_y_train)
        ...
        # remove the x feature test and training sets
        rm(data_x_test)
        rm(data_x_train)
        rm(featureData)
